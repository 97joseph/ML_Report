function varargout = displayBarValues(h,precision)
% displayBarValues;
% displayBarValues(h)
% displayBarValues(h,precision)
% t = displayBarValues(h,precision)
%
...
elseif isaType(h,'figure')
   ...
   for b =B'
           t = [t; {displayBarValues(b,precision)}]; % Return array of text objects
                                              % for each bar plot.
   end
    if nargout>0
        varargout{1}=t;
